...
def strip_latex(text):
    ...
    t = _re.sub(r'\\(small|normalsize|large|Large|LARGE|huge|Huge|tiny|scriptsize|footnotesize)\b', '', t)
...
- scope: Does it address a genuine scientific question with appropriate depth? Score scope based on the STRONGEST layer of the paper, not the most speculative. A paper that presents formal mathematics, empirical validation across multiple domains, AND speculative theoretical framework should be scored on the formal+empirical work (which is the core), not penalized for the speculative layer. Papers that explicitly separate conjecture from established results (e.g., labeling claims as "formal," "empirical," vs "conjectural") demonstrate intellectual honesty and should score 4-5. A paper spanning 10+ empirical domains with formal proofs is exceptional scope regardless of whether its theoretical interpretation is fully established. Score 1-2 ONLY if there is no coherent scientific question at all.
...
- citations: Does it engage with prior literature? Count the actual references listed in the References/Bibliography section. ABSOLUTE RULE -- you MUST follow this scoring: 1-2 references = score 1. 3-4 references = score 2. 5-7 references = score 3. 8-10 references = score 4. 11+ references = score 5. This scoring is based on QUANTITY of real references only. Do NOT adjust downward because "the scope demands more" -- that reasoning is explicitly prohibited. A paper with 7 references scores 3, period.
...
{"scores":{"scope":N,"claim":N,"structure":N,"clarity":N,"quantitative":N,"citations":N,"reproducibility":N,"good_faith":N},"strengths":["...","..."],"suggestions":["...","..."],"recommendation":"pass or return or block","summary":"One paragraph assessment"}""",
        ...
        try:
            ...
            pass
        # SERVER-SIDE CITATIONS OVERRIDE: count actual references and enforce minimum score
        import re as _re_override
        ref_count = 0
        # Count \bibitem entries
        ref_count += len(_re_override.findall(r'\\bibitem', body))
        # Count numbered references like [1], [2] in a references section
        ref_section = _re_override.split(r'(?i)(?:references|bibliography)', body)
        if len(ref_section) > 1:
            ref_text = ref_section[-1]
            # Count enumerated items or bracket references
            ref_count = max(ref_count, len(_re_override.findall(r'(?:^\s*\[?\d+\]?[\.\):]|\\item)', ref_text, _re_override.MULTILINE)))
        if ref_count == 0:
            # Try counting unique citation keys like [AuthorYear] or [1]-[7]
            cites = set(_re_override.findall(r'\[([^\]]{1,40})\]', body))
            ref_count = max(ref_count, len([c for c in cites if not c.replace('.','').replace('-','').isdigit() or int(c.split('.')[0].split('-')[0]) <= 50]))
        # Apply citation floor based on count
        citations_floor = 1
        if ref_count >= 11: citations_floor = 5
        elif ref_count >= 8: citations_floor = 4
        elif ref_count >= 5: citations_floor = 3
        elif ref_count >= 3: citations_floor = 2
        if scores.get("citations", 0) < citations_floor:
            old_cit = scores["citations"]
            scores["citations"] = citations_floor
            # Recalculate overall
            overall = round(sum(scores.values()) / len(scores) * 20)
            print(f"[CLAUDE] CITATIONS OVERRIDE: {old_cit} -> {citations_floor} (found {ref_count} refs, overall now {overall}%)")
...
def er_analysis(series):
    ...
    return {
        ...
        'details': {
            ...
            'series_preview': series[:300],
            'coherence_series': [round(c, 4) for c in adjacent][:200] if adjacent else [],
        },
    }
...
def extract_text_from_upload():
    """Extract text from an uploaded PDF or ZIP file. Used by AI manuscript tools before submission."""
    ...
    if fname.endswith(".zip"):
        ...
        text = (pkg["text"] + pkg["all_text"]).strip()
        # Also try to extract abstract
        abstract = extract_abstract_from_text(text)
        print(f"[EXTRACT] ZIP: {len(text)} chars, abstract: {len(abstract)} chars")
        return jsonify({"text": text[:200000], "abstract": abstract})
    elif fname.endswith(".pdf"):
        text = extract_pdf_text(file_bytes) or ""
        abstract = extract_abstract_from_text(text)
        print(f"[EXTRACT] PDF: {len(text)} chars, abstract: {len(abstract)} chars")
        return jsonify({"text": text[:200000], "abstract": abstract})
    elif fname.endswith((".tex", ".txt", ".md")):
...
def extract_abstract_from_text(text):
    """Extract abstract from manuscript text (TEX or PDF-extracted).
    Returns the abstract string or empty string if not found."""
    import re
    if not text:
        return ""
    # Try LaTeX: \begin{abstract} ... \end{abstract}
    m = re.search(r'\\begin\{abstract\}(.*?)\\end\{abstract\}', text, re.DOTALL)
    if not m:
        # Try: \textbf{Abstract} followed by \begin{quote} ... \end{quote}
        m = re.search(r'\\textbf\{Abstract\}.*?\\begin\{quote\}(?:\\small|\\normalsize)?\s*(.*?)\\end\{quote\}', text, re.DOTALL)
    if not m:
        # Try: Abstract heading followed by content until next section
        m = re.search(r'(?:Abstract|ABSTRACT)\s*[\}\s]*\n+(.*?)(?=\\section|\\begin\{|$)', text, re.DOTALL)
    if m:
        abstract = m.group(1).strip()
        # Apply full strip_latex for thorough cleanup
        abstract = strip_latex(abstract)
        abstract = re.sub(r'\s+', ' ', abstract).strip()
        if len(abstract) > 20:
            print(f"[ABSTRACT] Extracted from LaTeX: {len(abstract)} chars")
            return abstract[:3000]
    # Try PDF text: paragraph after "Abstract" heading
    patterns = [
        r'(?i)\bAbstract\b[\s.:_\-]*\n+(.*?)(?:\n\s*\n|\n(?:1[\s.]|I[\s.]|Introduction|Keywords|INTRODUCTION))',
        r'(?i)\bAbstract\b[\s.:_\-]*(.*?)(?:\n\s*\n)',
    ]
    for pat in patterns:
        ...
        if m:
            ...
            abstract = re.sub(r'\s+', ' ', abstract).strip()
            if len(abstract) > 50:
                print(f"[ABSTRACT] Extracted from PDF text: {len(abstract)} chars")
                return abstract[:3000]
    print("[ABSTRACT] Could not extract abstract from manuscript")
    return ""
...
def create_submission():
    try:
        ...
        if not payload["title"] or (not payload["abstract"] and not zip_text):
            return jsonify({"error": "Title required. Abstract required unless a manuscript file is attached."}), 400
        # Auto-extract abstract from manuscript if user didn't provide one
        if not payload["abstract"] and zip_text:
            extracted_abstract = extract_abstract_from_text(zip_text)
            if extracted_abstract:
                payload["abstract"] = extracted_abstract
                print(f"[SUBMIT] Auto-extracted abstract: {len(extracted_abstract)} chars")
            else:
                # Fallback: use first ~500 chars of body as abstract
                payload["abstract"] = zip_text[:500].strip()
                print(f"[SUBMIT] Abstract fallback: first 500 chars of body")
        # Always clean LaTeX from abstract before storing
        if payload["abstract"]:
            payload["abstract"] = strip_latex(payload["abstract"]).strip()
...
def ai_assist():
    ...
    # For upload mode, clean the full text for better AI analysis
    clean_full = strip_latex(full_text) if full_text and '\\' in full_text else full_text
    # Use full text if section text is empty (upload mode)
    effective_section = section_text if section_text else clean_full

    actions = {
        "improve_section": f"Improve this section of a scientific paper. Keep the author's voice but strengthen the scientific writing, clarity, and rigor. Section: {section}\n\nCurrent text:\n{effective_section[:8000]}",
        ...
        "strengthen_claims": f"Review this paper and identify claims that need stronger support. For each weak claim, suggest what evidence, analysis, or citation would strengthen it.\n\nTitle: {title}\nAbstract: {abstract}\nPaper:\n{effective_section[:12000]}",
    ...
    try:
        ...
        message = client.messages.create(
            model="claude-sonnet-4-20250514",
            max_tokens=2000,
            system="""You are a scientific writing assistant for OpenField, an independent open-access science journal. You help researchers improve their papers before submission.

OPENFIELD EDITORIAL PHILOSOPHY:
- OpenField welcomes independent researchers. Evaluate the science, not credentials.
- OpenField is NOT a blind review journal. Author names and affiliations are expected.
- The journal explicitly welcomes ambitious interdisciplinary work that spans formal mathematics, empirical data analysis, and speculative theoretical frameworks.
- Papers that explicitly separate their claim hierarchy (formal/proved, operational/defined, empirical/observed, conjectural/proposed) demonstrate intellectual honesty and scientific maturity. This is a STRENGTH, not a weakness.
- Score the work based on its STRONGEST layer, not its most speculative. A paper that proposes a theoretical framework AND provides rigorous empirical validation across multiple domains is demonstrating exceptional ambition, not overreach.
- Do NOT dismiss interdisciplinary connections as 'inconsistencies.' Bridging domains is the point. If a paper connects time series analysis to cosmological theory and clearly labels the cosmological part as conjecture, that is honest science.
- Focus on whether the math is sound, the data analysis is proper, and the claims are appropriately hedged at each level of the hierarchy.
- If the paper includes code, data, locked parameters, or supplementary files, credit these as evidence of reproducibility and rigor.

WHEN PROVIDING FEEDBACK:
- Be constructive, specific, and maintain the author's voice.
- Suggest improvements and explain why.
- Do not rewrite everything.
- Do not treat speculative theoretical interpretation as a flaw if the paper explicitly labels it as conjecture.
- Acknowledge what works well before suggesting changes.
- Name exactly what is missing or weak — do not just say 'add more detail.'""",
