...
app.config['REMEMBER_COOKIE_SECURE'] = os.getenv('RENDER', '') != ''
...
response.headers['Access-Control-Allow-Headers'] = 'Content-Type, Authorization, X-Re
...
response.headers['Access-Control-Allow-Headers'] = 'Content-Type, Authorization, X-Reques
response.headers['Access-Control-Allow-Methods'] = 'GET, POST, PUT, DELETE, OPTIONS'
response.headers['Access-Control-Allow-Credentials'] = 'true'
return response
...
def to_dict(self):
try: pc = Submission.query.filter_by(author_id=self.id).count()
except: pc = 0
try: rc = Review.query.filter_by(reviewer_id=self.id).count()
except: rc = 0
try: fc = self.followers.count() if hasattr(self, 'followers') else 0
except: fc = 0
return {'id': self.id, 'display_name': self.display_name, 'email': self.email,
'initials': self.initials, 'bio': self.bio, 'avatar_color': self.avatar_color
'role': self.role, 'reputation_score': round(self.reputation, 2),
...
STATUS_LABELS = {'submitted':'Submitted','desk_passed':'Under Review','in_discovery':'In
'under_review':'Under Review','published':'Published','desk_returned':'Revision Sugge
'revision_requested':'Revision Requested','declined':'Declined','contested':'Conteste
...
d = {'id':self.id,'blind_id':self.blind_id,'title':self.title,'abstract':(self.abstra
'status':self.status,'status_label':self.STATUS_LABELS.get(self.status, self.sta
'status_color':self.STATUS_COLORS.get(self.status, '#6b7db3'),
'category':{'name':self.category.name,'emoji':self.category.emoji} if self.categ
...
if uid:
d['user_liked'] = Like.query.filter_by(user_id=uid, submission_id=self.id).first(
d['user_bookmarked'] = Bookmark.query.filter_by(user_id=uid, submission_id=self.i
return d
class Like(db.Model):
id = db.Column(db.Integer, primary_key=True)
...
claims = ['we show','we find','i find','this paper','result shows','i measured','hypothes
...
rec = 'block' if is_spam else ('pass' if overall>=60 and scores['good_faith']>=3 else 're
return {'overall_score':overall,'recommendation':rec,'scores':scores,
'summary':'Ready for community review.' if rec=='pass' else 'Needs more developme
'encouragement':'Your curiosity is valued here.' if rec!='block' else 'We welcome
# -- Auth helpers --
...
def categories():
return jsonify({'categories':[{'id':c.id,'name':c.name,'emoji':c.emoji,'slug':c.slug} for
@app.route('/api/feed/discovery')
def discovery():
page = request.args.get('page',1,type=int)
q = Submission.query.filter(Submission.status.in_(['in_discovery','under_review','revisio
cat = request.args.get('category_id',type=int)
if cat: q = q.filter_by(category_id=cat)
search = request.args.get('q','').strip()
if search: q = q.filter(or_(Submission.title.ilike('%'+search+'%'),Submission.abstract.il
p = q.order_by(Submission.updated_at.desc()).paginate(page=page,per_page=20,error_out=Fal
uid = current_user.id if current_user.is_authenticated else None
return jsonify({'papers':[s.to_card(uid) for s in p.items],'total':p.total,'page':p.page}
@app.route('/api/feed/published')
def published():
p = Submission.query.filter_by(status='published').order_by(Submission.published_at.desc(
uid = current_user.id if current_user.is_authenticated else None
...
tags=d.get('tags',''), author_id=current_user.id, category_id=d.get('category_id',1))
desk = run_desk_review(sub.title, sub.abstract, sub.body_text)
sub.status = {'pass':'in_discovery','return':'desk_returned','block':'desk_blocked'}[desk
db.session.add(sub)
db.session.flush()
...
'body':c.body,'created_at':c.created_at.isoformat()} for c in Comment.query.filter_by
...
'comment_type':c.comment_type,'body':c.body,'created_at':c.created_at.isoformat()}}),
@app.route('/api/users/<int:uid>')
def get_user(uid):
u = User.query.get_or_404(uid)
d = u.to_dict()
d['papers'] = [s.to_card() for s in Submission.query.filter_by(author_id=uid).filter(
Submission.status.in_(['in_discovery','under_review','published'])).order_by(Submissi
return jsonify({'user':d})
@app.route('/api/notifications')
@api_login_required
def notifications():
ns = Notification.query.filter_by(user_id=current_user.id).order_by(Notification.created_
...
'unread_count':Notification.query.filter_by(user_id=current_user.id,is_read=False).co
@app.route('/api/stats')
def stats():
return jsonify({'published_count':Submission.query.filter_by(status='published').count(),
'discovery_count':Submission.query.filter(Submission.status.in_(['in_discovery','unde
...
return jsonify({'submissions':[{'id':s.id,'blind_id':s.blind_id,'title':s.title,
...
'comment_count':Comment.query.filter_by(submission_id=s.id).count()} for s in subs]})
...
d = request.get_json(silent=True) or {}
ns = d.get('status','')
allowed = ['in_discovery','under_review','published','desk_returned','declined','desk_blo
if ns not in allowed: return jsonify({'error':'Invalid status'}), 400
s.status = ns
...
return jsonify({'users':[{'id':u.id,'email':u.email,'display_name':u.display_name,
'role':u.role,'reputation':round(u.reputation,2),'is_banned':u.is_banned,
...
d = request.get_json(silent=True) or {}
r = d.get('role','member')
if r not in ['member','admin','reviewer']: return jsonify({'error':'Invalid role'}), 400
...
@admin_required
def reset_db():
# Delete all non-admin users and all submissions
...
# -- Seed --
def seed():
cats = [('foundations','Foundations of Physics','F'),('math-physics','Mathematical Physic
('nonlinear','Nonlinear Dynamics','N'),('stat-mech','Statistical Mechanics','S'),
('complex','Complex Systems','C'),('experimental','Experimental and Observational
