...
DEFAULT_ADMIN_EMAIL = os.getenv('DEFAULT_ADMIN_EMAIL', 'admin@journal.local').strip().lower()
DEFAULT_ADMIN_PASSWORD = os.getenv('DEFAULT_ADMIN_PASSWORD', 'change-me-now')
DEFAULT_ADMIN_NAME = os.getenv('DEFAULT_ADMIN_NAME', 'Founding Editor')
FORCE_DEFAULT_ADMIN_PASSWORD = os.getenv('FORCE_DEFAULT_ADMIN_PASSWORD', '1').strip().lower() in {'1', 'true', 'yes', 'on'}
...
def safe_password_matches(user, password):
    try:
        return bool(user and user.password_hash and user.check_password(password))
    except Exception:
        return False


def ensure_admin_account(force_password=False):
    admin = User.query.filter_by(email=DEFAULT_ADMIN_EMAIL).first()
    created = False
    changed = False
    if not admin:
        admin = User(email=DEFAULT_ADMIN_EMAIL, display_name=DEFAULT_ADMIN_NAME, role='admin')
        db.session.add(admin)
        created = True
        changed = True
    if not (admin.display_name or '').strip():
        admin.display_name = DEFAULT_ADMIN_NAME
        changed = True
    if admin.role != 'admin':
        admin.role = 'admin'
        changed = True
    if force_password or FORCE_DEFAULT_ADMIN_PASSWORD or not getattr(admin, 'password_hash', None):
...
def login():
    ...
    if email == DEFAULT_ADMIN_EMAIL:
        if not user:
            user = ensure_admin_account(force_password=True)
        elif pw == DEFAULT_ADMIN_PASSWORD and not safe_password_matches(user, pw):
            user = ensure_admin_account(force_password=True)

    if not user or not safe_password_matches(user, pw):
...
def seed():
    ...
    db.session.commit()
    ensure_admin_account(force_password=True)
