...
def run_claude_review(title, abstract, body):
    ...
    try:
        ...
        if len(body) <= MAX_BODY:
            ...
            for marker in ["References", "REFERENCES", "Bibliography", "BIBLIOGRAPHY"]:
                ...
                if idx > 0:
                    ...
                    break
            if ref_idx > 0:
                refs_section = body[ref_idx:][:8000]
                remaining = MAX_BODY - len(refs_section) - 200
                body_to_send = body[:remaining] + "\n\n[...truncated...]\n\n" + refs_section
            else:
                body_to_send = body[:MAX_BODY]
        ...
        message = client.messages.create(
            ...
            max_tokens=1500,
            system="""You are a rigorous desk reviewer for OpenField, an independent science journal. You evaluate manuscripts for scientific readiness. This journal welcomes independent researchers -- evaluate the science, not credentials. But standards are high: roughly 35% of submissions should be returned for revision.
...
{"scores":{"scope":N,"claim":N,"structure":N,"clarity":N,"quantitative":N,"citations":N,"anonymity":N,"good_faith":N},"strengths":["...","..."],"suggestions":["...","..."],"recommendation":"pass or return or block","summary":"One paragraph assessment"}""",
        ...
        score_vec = [scores.get(k, 3) for k in ["scope", "claim", "structure", "clarity", "quantitative", "citations", "anonymity", "good_faith"]]
        etce_meta = None
        try:
...
def process_zip_package(zip_bytes):
    ...
    try:
        ...
        names = zf.namelist()
        print(f"[ZIP] Processing package with {len(names)} files: {names[:20]}")
        for name in names:
            ...
            # Identify file types
            if low.endswith(".pdf"):
                if not result["pdf_bytes"] or "manuscript" in low or "paper" in low or "main" in low:
            ...
            elif low.endswith((".csv", ".tsv", ".dat", ".xls", ".xlsx")):
                result["data_files"].append(base)
                # Try to read CSV content for review context
                if low.endswith((".csv", ".tsv", ".dat")):
                    try:
                        content = data.decode("utf-8", errors="ignore")
                        # Only include first 2000 chars of data files
                        result["all_text"] += f"\n\n--- DATA FILE: {base} ---\n{content[:2000]}\n"
                    except Exception:
                        pass
                ...
                try:
                    ...
                    pass
            elif low.endswith((".txt", ".tex", ".md", ".bib")):
                try:
                    content = data.decode("utf-8", errors="ignore")
                    if low.endswith(".tex") and len(content) > len(result["text"]):
                        result["text"] = content
    ...
    for key in request.files:
        ...
        try:
            ...
            if fname.endswith(".pdf"):
                parts.append(extract_pdf_text(raw))
            else:
            ...
            if uploaded and uploaded.filename:
                ...
                if len(file_bytes) <= 10 * 1024 * 1024:  # 10MB limit
                    if fname.endswith(".zip"):
                        ...
                        print(f"[ZIP] Package loaded: {len(pkg['figures'])} figs, {len(pkg['data_files'])} data, {len(pkg['code_files'])} code")
