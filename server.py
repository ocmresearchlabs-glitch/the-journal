...
def to_dict(self):
    ...
    try:
        ...
        fc = 0
    return {
        'id': self.id, 'display_name': self.display_name, 'email': self.email,
...
def to_card(self, uid=None):
    ...
    d = {
        ...
        'category': {'name': self.category.name, 'emoji': self.category.emoji} if self.category else None,
...
submission_id=sub.id, decision=desk[‘recommendation’],
...
‘body’: c.body, ‘created_at’: c.created_at.isoformat()} for c in Comment.query.filter_by(submission_id=s.id).all()]
