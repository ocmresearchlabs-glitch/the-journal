...
**table_args** = (db.UniqueConstraint(‘user_id’,‘submission_id’),)
...
if search: q = q.filter(or_(Submission.title.ilike(’%’+search+’%’),Submission.abstract.ilike(’%’+search+’%’)))
...
d = request.get_json(silent=True) or {}
ns = d.get(‘status’,’’)
allowed = [‘in_discovery’,‘under_review’,‘published’,‘desk_returned’,‘declined’,‘desk_blocked’]
