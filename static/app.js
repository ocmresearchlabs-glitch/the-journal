...
document.documentElement.style.background = '#080c18';
document.body.style.margin = '0';
document.body.style.minHeight = '100%';
document.body.style.background = '#080c18';
...
function AuthScreen({ onAuth }) {
    ...
    return React.createElement("div", { style: { minHeight: '100vh', minHeight: '100dvh', background: 'linear-gradient(180deg,#080c18,#0d1225 30%,#0a0f1e)', color: '#edf1ff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 'max(40px, calc(env(safe-area-inset-top) + 24px)) 16px max(28px, calc(env(safe-area-inset-bottom) + 20px))', boxSizing: 'border-box' } },
...
function NotificationDropdown({ items, onClose, onMarkRead, onOpenProfile }) {
    return React.createElement("div", { style: { position: 'absolute', right: 0, top: 'calc(100% + 10px)', width: 310, maxHeight: 360, overflowY: 'auto', background: '#0d1225', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, boxShadow: '0 18px 40px rgba(0,0,0,0.35)', padding: 10, zIndex: 200 } },
...
function BuilderScreen({ categories, useApi, onToast, onOpenTool, pushProfile }) {
    ...
    return React.createElement("div", { style: { maxWidth: 680, margin: '0 auto', animation: 'fadeIn .25s' } },
        React.createElement("div", { style: { marginBottom: 14, padding: '14px 16px', borderRadius: 12, background: 'linear-gradient(135deg,rgba(94,168,255,0.05),rgba(94,168,255,0.02))', border: '1px solid rgba(94,168,255,0.08)' } },
...
function ProfileScreen({ user, isSelf, useApi, onLogout, onToast, onProfileSaved }) {
    ...
    return React.createElement("div", { style: { maxWidth: 520, margin: '0 auto', animation: 'fadeIn .25s' } },
...
function App() {
    ...
    return React.createElement("div", { style: { minHeight: '100vh', minHeight: '100dvh', width: '100%', overflowX: 'hidden', background: 'linear-gradient(180deg,#080c18,#0d1225 30%,#0a0f1e)', color: '#edf1ff', fontFamily: '-apple-system,BlinkMacSystemFont,\'Helvetica Neue\',sans-serif' } },
        ...
        React.createElement("header", { ref: headerRef, style: { position: 'sticky', top: 0, zIndex: 100, background: 'rgba(8,12,24,0.92)', backdropFilter: 'blur(16px)', borderBottom: '1px solid rgba(255,255,255,0.06)', paddingTop: 'env(safe-area-inset-top)' } },
            React.createElement("div", { style: { width: '100%', boxSizing: 'border-box', padding: '10px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 } },
                React.createElement("div", { onClick: () => { setView('discover'); setSelectedBlindId(null); }, style: { cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, flex: '1 1 auto', minWidth: 0 } },
                    ...
                    React.createElement("span", { style: { fontWeight: 800, fontSize: 15, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' } }, "The Journal")),
                React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 6, position: 'relative', flexShrink: 0 } },
                    React.createElement("button", { onClick: () => { setView('builder'); setSelectedBlindId(null); }, style: { display: 'flex', alignItems: 'center', gap: 4, padding: '6px 12px', borderRadius: 8, background: 'linear-gradient(135deg,#5ea8ff,#3d8be0)', border: 'none', color: '#071120', fontWeight: 700, fontSize: 12, cursor: 'pointer', whiteSpace: 'nowrap' } }, "+ Submit"),
                    ...
                    React.createElement("div", { onClick: () => { setSelectedProfile(user); setView('profile'); }, style: { cursor: 'pointer' } },
                        React.createElement(Av, { user: user, size: 28 })),
        ...
        React.createElement("main", { style: { width: '100%', boxSizing: 'border-box', padding: '12px 12px calc(88px + env(safe-area-inset-bottom))' } },
